...
import { DefaultEventPriority, IdleEventPriority, ConcurrentRoot, LegacyRoot } from 'react-reconciler/constants';
const NO_CONTEXT = {};
...
function createReactNoop(reconciler, useMutation) {
  ...
  const hostConfig = useMutation ? { ...sharedHostConfig,
    ...
    cloneHiddenInstance(instance, type, props, internalInstanceHandle) {
      const clone = cloneInstance(instance, null, type, props, props, internalInstanceHandle, true, null);
      clone.hidden = true;
      return clone;
    },
  ...
  const ReactNoop = {
    ...
    createRoot(options) {
      ...
      const fiberRoot = NoopRenderer.createContainer(container, ConcurrentRoot, null, null, false, '', onRecoverableError, options && options.transitionCallbacks ? options.transitionCallbacks : null);
      return {
        _Scheduler: Scheduler,
    ...
    createLegacyRoot() {
      ...
      const fiberRoot = NoopRenderer.createContainer(container, LegacyRoot, null, null, false, '', onRecoverableError);
      return {
        _Scheduler: Scheduler,
    ...
    createPortal(children, container, key = null) {
