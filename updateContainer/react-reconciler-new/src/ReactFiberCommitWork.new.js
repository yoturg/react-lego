...
import { captureCommitPhaseError, resolveRetryWakeable, markCommitTimeOfFallback, enqueuePendingPassiveProfilerEffect, restorePendingUpdaters, addTransitionStartCallbackToPendingTransition, addTransitionCompleteCallbackToPendingTransition } from './ReactFiberWorkLoop.new';
...
import { clearTransitionsForLanes } from './ReactFiberLane.new'; // Used during the commit phase to track the state of the Offscreen component stack.
    ...
    case HostRoot:
      {
        ...
        if (enableTransitionTracing) {
          ...
          const root = finishedWork.stateNode;
          let incompleteTransitions = root.incompleteTransitions; // Initial render
          ...
          if (incompleteTransitions === null || incompleteTransitions.size === 0) {
        ...
        if (enableTransitionTracing) {
          ...
          if (queue !== null) {
            if (isFallback) {
              ...
              let rootIncompleteTransitions = finishedRoot.incompleteTransitions; // We lazily instantiate transition tracing relevant maps
              ...
              if (transitions !== null) {
                transitions.forEach(transition => {
                  ...
                  prevTransitions.add(transition); // Add the root transition's pending suspense boundary set to
                  ...
                  if (rootIncompleteTransitions !== null) {
                    ...
                    instance.pendingMarkers.add(rootIncompleteTransitions.get(transition));
                  }
                });
              }
            }
            ...
            if (instance.pendingMarkers === null || instance.pendingMarkers.size === 0) {
              finishedWork.updateQueue = null;
            }
          }
        }
...
export { commitPlacement, commitAttachRef };