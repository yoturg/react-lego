...
import { markComponentRenderStarted, markComponentRenderStopped } from './ReactFiberDevToolsHook.new';
...
import { processUpdateQueue, cloneUpdateQueue, initializeUpdateQueue } from './ReactFiberClassUpdateQueue.new';
import { NoLane, NoLanes, SyncLane, OffscreenLane, DefaultHydrationLane, SomeRetryLane, NoTimestamp, includesSomeLane, laneToLanes, removeLanes, mergeLanes, getBumpedLaneForHydration } from './ReactFiberLane.new';
import { ConcurrentMode, NoMode, ProfileMode } from './ReactTypeOfMode';
import { shouldSetTextContent, isSuspenseInstancePending, isSuspenseInstanceFallback, getSuspenseInstanceFallbackErrorDetails, registerSuspenseInstanceRetry, supportsHydration, isPrimaryRenderer } from './ReactFiberHostConfig';
...
import { getIsHydrating, enterHydrationState, reenterHydrationStateFromDehydratedSuspenseInstance, resetHydrationState, tryToClaimNextHydratableInstance, warnIfHydrating, queueHydrationError } from './ReactFiberHydrationContext.new';
...
import { retryDehydratedSuspenseBoundary, scheduleUpdateOnFiber, renderDidSuspendDelayIfPossible, markSkippedUpdateLanes, getWorkInProgressRoot, pushRenderLanes } from './ReactFiberWorkLoop.new';
...
import { requestCacheFromPool, pushRootTransition, getSuspendedCache, pushTransition, getOffscreenDeferredCache, getSuspendedTransitions } from './ReactFiberTransition.new';
...
function updateOffscreenComponent(current, workInProgress, renderLanes) {
  ...
  if (nextProps.mode === 'hidden' || enableLegacyHidden && nextProps.mode === 'unstable-defer-without-hiding') {
    ...
    if (prevState !== null) {
      ...
      pushTransition(workInProgress, prevCachePool, transitions); // Since we're not hidden anymore, reset the state
